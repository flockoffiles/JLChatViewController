...
open class JLChatImageView: UIImageView {
    ...
    */
    ...
    private func cutImage(image:UIImage)->UIImage {
        ...
        let rect: CGRect = CGRect(origin: CGPoint(x:posX, y:posY), size: CGSize(width: newWidth, height: newHeight))
     ...
     */
    open func addImage(image:UIImage,ApplyingMask mask:UIImage?){
        
        if useBubbleForm,let mask = mask{
            
            DispatchQueue.global(qos: .userInitiated).async() { // 1
                let imageReference = self.cutImage(image: image).cgImage!
